...
The chronology of news defines an interesting storyline that deserve a qualitative analysis.But we can also try to have a quantitative view of the proportion of news related to the new pandemics during the period of observation.  

```{r}
w<-docvars(qd) %>% mutate(time = cut(chrono$date,breaks = "month"),
                          topic = as.factor(chrono$nb_cov !=0)) %>%
                  count(time,topic) %>%
                  plot_ly(x = ~time, y = ~n, color = ~topic) %>%
                        add_bars() %>%
