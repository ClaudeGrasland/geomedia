...
df1<-fread("data/source/geomedia/es_BOL_lajord_int.csv", encoding = "UTF-8")
df1$media<-"es_BOL_lajord"


df2<-fread("data/source/geomedia/es_BOL_larazo_int.csv", encoding = "UTF-8")
df2$media<-"es_BOL_larazo"

df3<-fread("data/source/geomedia/es_BOL_patria_int.csv", encoding = "UTF-8")
df3$media<-"es_BOL_patria"

df4<-fread("data/source/geomedia/es_BOL_tiempo_int.csv", encoding = "UTF-8")
df4$media<-"es_BOL_tiempo"
...
mycomps<-c("china sea", "indian ocean", "american continent", "american countries")
...
We decide here to use lower case transformation. 

```{r dico pandemic}
# define keywords
label <- c("migrant*", "emigrant*", "immigrant*", "migrat*", "refugiad*", "asilo")
...
hc2<-hc %>% filter(where1 !="_no_", where2 !="_no_") %>% filter(where1 !="FRA", where2 !="FRA")
