...
df1<-fread("data/source/geomedia/it_ITA_corrie_int.csv", encoding = "UTF-8")
df1$media<-"it_ITA_corrie"


df2<-fread("data/source/geomedia/it_ITA_repubb_int.csv", encoding = "UTF-8")
df2$media<-"it_ITA_repubb"
...
extract_tags <- function(qd = qd,                      # the corpus of interest
                         lang = "it",                  # the language to be used
...
```{r dico pandemic}
# define keywords
label <- c("migrant*", "immigrant*", "asilo*", "refugiat*")
code  <- c("migr","migr","refu", "refu")
lang  <- rep("it", length(label))
...
hc2<-hc %>% filter(where1 !="_no_", where2 !="_no_") %>% filter(where1 !="ITA", where2 !="ITA")
...
=> NE PAS EXECUTER (effectif trop faible)
